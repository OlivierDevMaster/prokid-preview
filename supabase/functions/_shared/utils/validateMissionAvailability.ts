...
export interface ValidationResult {
  constrainedSchedules?: MissionSchedule[];
  isValid: boolean;
...
export function validateMissionAvailability(
  ...
  const constrainedSchedules: MissionSchedule[] = [];
  ...
  for (
    ...
    // Constrain the RRULE for this schedule (same as what will be stored in DB)
    try {
      ...
      // If we can't constrain the RRULE, that's a critical error
      violations.push({
        mission_occurrence_end: '',
        mission_occurrence_start: '',
        mission_schedule_index: scheduleIndex,
        reason: `Invalid RRULE in mission schedule at index ${scheduleIndex}: ${String(rruleError)}`,
      });
    }
  }
  ...
  return {
    constrainedSchedules:
      violations.length === 0 ? constrainedSchedules : undefined,
...
function generateAvailabilityOccurrences(
  ...
  if (
    ...
    typeof (availabilityRule as RRulePkg.RRuleSet).rrules === 'function'
  ) {
    // It's an RRuleSet - get dtstart/until from the first RRule
    const rruleSet = availabilityRule as RRulePkg.RRuleSet;
    ...
    const rrule = availabilityRule as RRulePkg.RRule;
...
function generateMissionOccurrences(
  ...
  if (
    ...
    typeof (missionRule as RRulePkg.RRuleSet).rrules === 'function'
  ) {
    // It's an RRuleSet - get dtstart/until from the first RRule
    const rruleSet = missionRule as RRulePkg.RRuleSet;
    const rules = rruleSet.rrules();
    ...
    // It's a regular RRule
    const rrule = missionRule as RRulePkg.RRule;
