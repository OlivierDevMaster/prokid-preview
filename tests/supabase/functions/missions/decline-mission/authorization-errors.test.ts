...
describe('Mission decline authorization errors', () => {
  ...
  it('should reject decline when user is not the assigned professional', async () => {
    // Arrange
    ...
    // Act - Try to decline using structure token (not professional)
  ...
  it('should reject decline when mission is already accepted', async () => {
    ...
    // Act - Try to decline an already accepted mission
    const { data, response } = await apiHelper.invokeEndpoint({
      method: 'POST',
      name: 'missions',
      path: `/${createdMission.id}/decline`,
  ...
  it('should reject decline when mission does not exist', async () => {
    // Arrange
    fixture = await fixtureBuilder.createStructureWithProfessionalMember();
    ...
    const { data, response } = await apiHelper.invokeEndpoint({
      method: 'POST',
      name: 'missions',
      path: `/${nonExistentMissionId}/decline`,
      token: fixture.professionalToken!,
    });
