...
export class TriggerTestFixtureBuilder {
  ...
  async createMembership(): Promise<TriggerTestFixture> {
    ...
    const { data: professionalSignInData } =
      await authClient.auth.signInWithPassword({
        email: professionalEmail,
        password: 'testpassword123',
      });

    const { data: structureSignInData } =
      await authClient.auth.signInWithPassword({
        email: structureEmail,
        password: 'testpassword123',
      });
    ...
    return {
      ...
      professionalId,
      professionalToken: professionalSignInData?.session?.access_token,
      structureId,
      structureToken: structureSignInData?.session?.access_token,
