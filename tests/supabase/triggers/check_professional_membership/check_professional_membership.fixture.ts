...
export class TriggerTestFixtureBuilder {
  ...
  async createMissionWithMember(): Promise<TriggerTestFixture> {
    const fixture = await this.createStructureWithMember();
...
RRULE:FREQ=WEEKLY;BYDAY=MO,WE,FR`;
  ...
  async createStructureWithMember(): Promise<TriggerTestFixture> {
  ...
  async createStructureWithoutMember(): Promise<TriggerTestFixture> {
    const structureEmail = `test-structure-${Date.now()}@example.com`;
    const professionalEmail = `test-professional-${Date.now()}@example.com`;
