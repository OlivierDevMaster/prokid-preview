...
export class TriggerTestFixtureBuilder {
  ...
  async createInvitationWithExistingMembership(): Promise<TriggerTestFixture> {
    const fixture = await this.createPendingInvitation();

    await this.adminClient.from('structure_members').insert({
  ...
  async createPendingInvitation(): Promise<TriggerTestFixture> {
